...
int main()
{
    ...
    while (!WindowShouldClose())
    {
        ...
        mousePosition = Vector2Clamp(mousePosition, {0, 0}, {1920, 1080}); // probably unoptimal
        ...
        if (IsMouseButtonDown(MOUSE_BUTTON_LEFT))
        {
            ...
            // one important thing to remember is that the compiler is so meticulously optimised that all of these
            // approaches act a lot different from what you expect, they are all very optimised
        }

        // delete sandels
        // 'const' (constant) makes it so the Sandel object sndl references or is a copy of cannot be modified.
        // 'Sandel sndl' (pass by value), sndl is a copy of the Sandel object,
        // editing edits a copy contained inside the lambda.
        // 'Sandel& sndl' (pass by reference), sndl is a direct reference to the Sandel object,
        // although uneditable due to it being made a constant with 'const'.
